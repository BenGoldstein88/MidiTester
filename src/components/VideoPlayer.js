...
export default class VideoPlayer extends React.Component {
  ...
  toArrayBuffer(buffer) {
    ...
    console.log("ab: ", ab)
    console.log(buffer[0], buffer[10])
  ...
  uploadFile(fileEvent) {
    console.log("fileEvent: ", fileEvent, typeof fileEvent)
    this.playMidiFile(fileEvent)
  }

  playMidiFile(fileEvent) {
    var noteHash = {
  0: "C-2",
  1: "C#/Db-2",
  2: "D-2",
  ...
  127: "G8"
    ...
    fileEvent = fileEvent.replace(/\s+/g, '');
    // console.log("SampleMIDI: ", SampleMIDI)

    var ab = new ArrayBuffer(fileEvent.length/2)
    console.log("ab: ", ab)
    var bufferedMIDI = new Uint8Array(ab)

    var j = 0;
    for(var i = 0; i<fileEvent.length-1; i = i+2) {
      var hexString = '0x' + fileEvent.slice(i, i+2)
      var hexByte = parseInt(hexString)

      if(hexByte < 0) {
        console.log(hexString, hexByte)
      }
...
render() {
  ...
  return (
    ...
    <MidiDropZone onFileUpload={this.handleFileUpload} />
